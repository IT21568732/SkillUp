...
export default function Login() {
  ...
  const handleLogin = (e) => {
    ...
    axios
      .post("http://localhost:8002/auth/login", user)
      .then((response) => {
    ...
    <div className="font-poppins bg-white text-blue-500">
      ...
      >
        <div className="absolute top-0 left-0 right-0 flex justify-between px-4 py-2">
          <a
            href="/"
            className="mt-3 ml-12 md:mt-5 md:ml-4 flex text-4xl font-bold text-blue-500 underline "
          >
            SkillUP
          </a>
          <div className="absolute right-0 flex justify-between px-4 py-2 space-x-4 top-4 ">
            ...
            <Link
              className="hidden sm:block px-4 py-2 font-semibold bg-blue-500 hover:bg-blue-600 text-white rounded-lg"
              to={"/register"}
            >
              Signup
            </Link>
        ...
        <div className="w-full max-w-sm p-8 rounded-lg shadow-2xl md:max-w-md mt-20 mb-10 lg:mt-12 lg:mb-5 ">
          ...
          <form className="space-y-4" onSubmit={handleLogin}>
            <div>
              <label
                className="block mb-1 text-lg font-semibold md:text-xl"
                htmlFor="email"
              >
                Email
              </label>
              <input
                className="w-full px-3 py-2 text-base border border-blue-300 rounded-md md:text-lg focus:outline-none focus:border-blue-500"
                type="email"
                id="email"
                name="email"
            ...
            <div className="mt-4 text-center ">
              ...
              <a href="/register" className="thover:underline">
                Signup here
              </a>
            </div>
          </form>
